...
@Component({
  selector: 'app-order-receipt-details',
  templateUrl: './order-receipt-details.component.html',
  styleUrls: ['./order-receipt-details.component.scss'],
})
export class OrderReceiptDetailsComponent extends BaseListComponent
  implements OnInit {
  ...
  constructor(
    ...
    private orderReceiptServ: StkTransactionService
  ) {
    super(toast, spinner, orderReceiptServ);
  ...
  retrieveStkTransDetails() {
    this.spinner.show();
    this.orderReceiptServ
      .findWithSearchPojo(this.serviceURL, this.searchingObject)
      .subscribe(
        (data: any) => {
          this.spinner.hide();
  ...
  downLoadStkTransReceipt(row: any) {
    ...
    this.orderReceiptServ
      .downloadFileWithSearchParam(this.serviceURL, downLoadSearchObject)
      .subscribe(
        (res) => {
